...
# ─── Configuration ─────────────────────────────────────────────
MQTT_BROKER = "127.0.0.1"          # localhost — Mosquitto runs here
MQTT_PORT = 1883
TEAM_ID = "superstars"
MQTT_TOPIC = f"vision/{TEAM_ID}/movement"

WS_HOST = "0.0.0.0"                # listen on all interfaces
WS_PORT = 9003                      # WebSocket port
...
async def ws_handler(websocket, path=None):
    ...
    # Send cached latest message to new clients
...
async def broadcast(message: str):
    ...
    if not connected_clients:
        return
    tasks = [asyncio.ensure_future(_safe_send(ws, message)) for ws in connected_clients.copy()]
