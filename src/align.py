# src/align.py 
""" 
Alignment demo using your WORKING pipeline: - Haar face detection (fast) - MediaPipe FaceMesh -> 5 keypoints (stable) - ArcFace-style 5pt alignment -> 112x112 (or any size you set) 
...
def main(
    ...
    # Use specified backend when provided (e.g. cv2.CAP_DSHOW on Windows)
    if backend is None:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Alignment demo (choose camera)")
    parser.add_argument("--cam", "-c", type=int, default=0, help="Camera index (0,1,2,...)")
    parser.add_argument("--dshow", action="store_true", help="Use DirectShow backend on Windows (cv2.CAP_DSHOW)")
    parser.add_argument("--size", "-s", default="112x112", help="Output size WxH (e.g. 112x112)")
    ...
    backend = cv2.CAP_DSHOW if args.dshow else None
    main(cam_index=args.cam, out_size=(w, h), mirror=(not args.no_mirror), backend=backend)