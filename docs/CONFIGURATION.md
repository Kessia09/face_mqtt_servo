...
## 2. VPS â€” WebSocket Relay

Upload the `server/` folder to the VPS (e.g., via `scp`):
```bash
# From your PC:
...
**Camera index:** If `CAMERA_INDEX = 2` doesn't work, edit `camera_pc/config.py` and try `0` or `1`.

---

## 4. ESP8266 â€” Flash MicroPython
...
Or use **Thonny IDE** â†’ Open files â†’ Save to MicroPython device.
...
D4 (GPIO2)    â”€â”€â”€â”€â”€â”€â†’    Signal (Orange/Yellow wire)
3V3 or VIN    â”€â”€â”€â”€â”€â”€â†’    VCC    (Red wire)
GND           â”€â”€â”€â”€â”€â”€â†’    GND    (Brown wire)
```
...
1. **VPS:** Start Mosquitto â†’ Start `ws_relay.py`
2. **ESP8266:** Power on (auto-connects WiFi + MQTT)
3. **PC:** Run `python -m camera_pc.main`
