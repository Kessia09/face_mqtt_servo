# Setup & Testing Commands â€” Complete Reference

All commands for every component, in execution order.

**Submission / live demo:** For the assignment, the broker runs on the VPS. Set **PC** and **ESP8266** to use the VPS IP as the MQTT broker: `MQTT_BROKER_IP` in `camera_pc/config.py` and `MQTT_BROKER` in `esp8266/config.py` must be your VPS IP (e.g. `157.173.101.159`), not localhost or your PCâ€™s LAN IP.
...
# scp -r server/ user305@157.173.101.159:~/server/

# Start WebSocket relay (on VPS)
python3 ~/server/ws_relay.py
...
```bash
# Subscribe (run on VPS or PC â€” shows incoming messages):
mosquitto_sub -h 157.173.101.159 -t "vision/superstars/movement" -v

# Simulate MOVE_LEFT:
mosquitto_pub -h 157.173.101.159 -t "vision/superstars/movement" \
...
For submission, set `MQTT_BROKER_IP` in `camera_pc/config.py` to your VPS IP (e.g. `157.173.101.159`).
...
For submission, set `MQTT_BROKER` in `esp8266/config.py` to your VPS IP (e.g. `157.173.101.159`), not your PCâ€™s LAN IP.
